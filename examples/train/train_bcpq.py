...
from examples.configs.bcpq_configs import BCPQ_DEFAULT_CONFIG, BCPQTrainConfig
from osrl.algorithms import BCPQ, BCPQTrainer
from osrl.common import TransitionDataset
from osrl.common.exp_util import auto_name, seed_all


@pyrallis.wrap()
def train(args: BCPQTrainConfig):
    # update config
    cfg, old_cfg = asdict(args), asdict(BCPQTrainConfig())
    differing_values = {key: cfg[key] for key in cfg.keys() if cfg[key] != old_cfg[key]}
    cfg = asdict(BCPQ_DEFAULT_CONFIG[args.task]())
    cfg.update(differing_values)
    args = types.SimpleNamespace(**cfg)

    # setup logger
    default_cfg = asdict(BCPQ_DEFAULT_CONFIG[args.task]())
